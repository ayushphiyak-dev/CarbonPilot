/**
 * CarbonPilot Main Application Logic
 * Handles state management, UI reactivity, and footprint calculations.
 */
'use strict';
...
/**
 * Loads application state from localStorage or returns default state.
...
/**
 * Recalculates the user's carbon footprint based on assessment inputs.
 * Updates global categories and total footprint values.
 */
function recalc(){
  const a=S.assess;
...
/* THEME */
/**
 * Initializes the theme (light/dark) based on localStorage or system preference.
 */
function initTheme(){
...
/* ROUTING */
/**
 * Navigates to a specific "page" within the SPA.
 * @param {string} id - The DOM ID of the target page.
 */
function go(id){
...
/**
 * Initializes Chart.js graphs for the dashboard and simulator.
 */
function initCharts(){
  if(typeof Chart==='undefined') return;
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = {
    DS, ALL_ACTIONS, CAR_FACTOR, DIET_BASE, WASTE_MULT, HEAT_KG, SH_FACTOR,
    loadState, save, recalc, initTheme, applyTheme, toggleTheme, go,
    observeReveals, updateChartColors, initCharts, renderDash,
    restoreAssessForm, renderAll, updateSim, restoreSim, resetSim,
    renderActions, renderProgress, renderCoach,
    openAddActionModal, closeAddActionModal,
    getAI, sendChat, quickChat, toggleAction,
    assessNext, assessBack, showStep, readStep, saveAssessment,
    getFp: () => fp, getCats: () => cats, getState: () => S,
  };
}
