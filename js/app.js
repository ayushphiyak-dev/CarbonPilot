/* ============================================================
   CARBONPILOT — Application Logic + Theme Toggle
   ============================================================ */
...
function updateChartColors() {
  ...
  Object.values(charts).forEach(chart => {
    if (chart) {
      chart.options.scales.x && (chart.options.scales.x.grid.color = c.grid);
      chart.options.scales.y && (chart.options.scales.y.grid.color = c.grid);
      chart.options.plugins.tooltip.backgroundColor = c.bg === '#1C1C1E' ? '#2C2C2E' : '#FFFFFF';
      chart.options.plugins.tooltip.titleColor = c.bg === '#1C1C1E' ? '#FFFFFF' : '#1C1C1E';
      chart.options.plugins.tooltip.bodyColor = c.bg === '#1C1C1E' ? '#A1A1A6' : '#6E6E73';
      chart.options.plugins.tooltip.borderColor = c.bg === '#1C1C1E' ? '#38383A' : '#E8E8ED';
      chart.update();
    }
  });
}
  ...
  categories.forEach(c => {
    ...
    div.innerHTML = `
      <div class="flex justify-between" style="margin-bottom:8px;font-size:14px">
        <span style="font-weight:600;color:var(--text)">${c.label}</span>
